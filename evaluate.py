...
if __name__ == "__main__":
    ...
    if args.checkpoint_path != "":
        if args.model_name == "ucaps":
            ...
            net = ModifiedUCaps3D.load_from_checkpoint(
                args.checkpoint_path,
                val_patch_size=args.val_patch_size,
                sw_batch_size=args.sw_batch_size,
                overlap=args.overlap,
    ...
    asd_metric = SurfaceDistanceMetric(include_background=False, reduction="none", get_not_nans=False)
    ...
    for i, data in enumerate(tqdm(val_loader)):
        ...
        asd_metric(y_pred=val_outputs, y=labels)
    ...
    print_metric("ASD", asd_metric.aggregate().cpu().numpy(), reduction=reduction)
